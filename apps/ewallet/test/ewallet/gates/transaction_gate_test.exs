defmodule EWallet.TransactionGateTest do
  ...
  describe "process_with_addresses/1" do
    ...
    def insert_transfer_with_addresses(%{
      ...
      {:ok, transfer} =
        Transfer.get_or_insert(%{
          ...
          from_minted_token_uuid: token.uuid,
          to: balance2.address,
          to_amount: 100 * token.subunit_to_unit,
          to_minted_token_uuid: token.uuid,
          metadata: metadata,
          payload: attrs,
    ...
    test "creates and confirms a transfer when idempotency token does not exist" do
      ...
      {status, _transfer, _balances, _minted_token} =
        TransactionGate.process_with_addresses(attrs)
  ...
  describe "process_credit_or_debit/1" do
    ...
    def insert_debit_credit_transfer(%{
      ...
      {:ok, transfer} =
        Transfer.get_or_insert(%{
          ...
          from_amount: 100_000,
          from_minted_token_uuid: inserted_token.uuid,
          to: Account.get_primary_balance(inserted_token.account).address,
          to_amount: 100_000,
          to_minted_token_uuid: inserted_token.uuid,
          metadata: metadata,
          payload: attrs,
          ledger_response: response,
          status: status,
    ...
    test "resend the request to the ledger when idempotency token is present and
      ...
      {status, transfer, _balances, _minted_token} =
