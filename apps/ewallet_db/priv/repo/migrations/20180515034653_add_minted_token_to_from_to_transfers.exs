defmodule EWalletDB.Repo.Migrations.AddMintedTokenToFromToTransfers do
  use Ecto.Migration
  import Ecto.Query
  alias Ecto.MigrationError
  alias EWalletDB.Repo

  def up do
    add_fields()
    flush()
    populate_fields()
    turn_off_nullables()
    remove_old_fields()
  end

  def add_fields do
    # Fields are added with `null:true` so we can populate data in.
    alter table(:transfer) do
      add :from_amount, :decimal, precision: 81,
                                  scale: 0,
                                  null: true

      add :from_minted_token_uuid, references(:minted_token, column: :uuid, type: :uuid),
                                   null: true

      add :to_amount, :decimal, precision: 81,
                                scale: 0,
                                null: true

      add :to_minted_token_uuid, references(:minted_token, column: :uuid, type: :uuid),
                                 null: true
    end
  end

  defp populate_fields do
    query = from(t in "transfer", update: [set: [from_amount: t.amount,
                                                 from_minted_token_uuid: t.minted_token_uuid,
                                                 to_amount: t.amount,
                                                 to_minted_token_uuid: t.minted_token_uuid]])

    Repo.update_all(query, [])
  end

  defp turn_off_nullables do
    alter table(:transfer) do
      modify :from_amount, :decimal, null: false
      modify :from_minted_token_uuid, :uuid, null: false
      modify :to_amount, :decimal, null: false
      modify :to_minted_token_uuid, :uuid, null: false
    end
  end

  defp remove_old_fields do
    alter table(:transfer) do
      remove :amount
      remove :minted_token_uuid
    end
  end

  def down do
    raise MigrationError, message: "This migration cannot be rolled back due to potential loss
                                   of transfer data."
  end
end
